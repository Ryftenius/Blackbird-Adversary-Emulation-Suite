...
typedef NTSTATUS(NTAPI* AES_NT_QUERY_VIRTUAL_MEMORY_FN)(_In_ HANDLE ProcessHandle, _In_opt_ PVOID BaseAddress,
...
typedef NTSTATUS(NTAPI* AES_NT_OPEN_PROCESS_FN)(_Out_ PHANDLE ProcessHandle, _In_ ACCESS_MASK DesiredAccess,
                                                      _In_ POBJECT_ATTRIBUTES ObjectAttributes, _In_ PVOID ClientId);

typedef struct _AES_CLIENT_ID
{
    HANDLE UniqueProcess;
    HANDLE UniqueThread;
} AES_CLIENT_ID;

#define AES_MEMORY_BASIC_INFORMATION_CLASS 0ul
#define AES_STATUS_SUCCESS ((NTSTATUS)0)
...
int __cdecl main(int argc, char** argv)
{
    ...
    DWORD targetPid = 0;
    HMODULE ntdll;
    AES_NT_QUERY_VIRTUAL_MEMORY_FN ntQueryVirtualMemory;
    AES_NT_OPEN_PROCESS_FN ntOpenProcess;
    DWORD ssnQueryVm = 0;
    ...
    SIZE_T outLen = 0;
    NTSTATUS statusWinApi = AES_STATUS_SUCCESS;
    NTSTATUS statusDirect = AES_STATUS_SUCCESS;
    NTSTATUS statusOpenLegit = AES_STATUS_SUCCESS;
    NTSTATUS statusOpenDirect = AES_STATUS_SUCCESS;
    AES_NT_QUERY_VIRTUAL_MEMORY_FN directQueryVmCall;
    AES_NT_OPEN_PROCESS_FN directOpenProcessCall;
    ...
    ntQueryVirtualMemory = (AES_NT_QUERY_VIRTUAL_MEMORY_FN)GetProcAddress(ntdll, "NtQueryVirtualMemory");
    if (ntQueryVirtualMemory == NULL)
    {
        printf("GetProcAddress(NtQueryVirtualMemory) failed err=%lu\n", GetLastError());
        return 1;
    }
    ntOpenProcess = (AES_NT_OPEN_PROCESS_FN)GetProcAddress(ntdll, "NtOpenProcess");
    ...
    directQueryVmCall = (AES_NT_QUERY_VIRTUAL_MEMORY_FN)(void*)g_SyscallStubQueryVm;
    directOpenProcessCall = (AES_NT_OPEN_PROCESS_FN)(void*)g_SyscallStubOpenProcess;

    {
        ...
        statusWinApi = ntQueryVirtualMemory(GetCurrentProcess(), (PVOID)(ULONG_PTR)&main,
                                            AES_MEMORY_BASIC_INFORMATION_CLASS, &mbi, sizeof(mbi), &outLen);
